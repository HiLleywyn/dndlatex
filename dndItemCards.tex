...
\usepackage{adjustbox} % Auto-scale long descriptions
...
\begin{document}
% Slightly smaller default font so text fits comfortably
...
\begin{tikzpicture}
    ...
    \foreach \filename in \cardfiles {%
        ...
        \pgfmathtruncatemacro{\i}{mod(\index,9) / 3}  % Row index within page
        \pgfmathtruncatemacro{\j}{mod(\index, 3)} % Column index

        % Start a new page every nine cards
        \ifnum\index>0
            \pgfmathtruncatemacro{\r}{mod(\index,9)}
            \ifnum\r=0
                \end{tikzpicture}
                \newpage
                \begin{tikzpicture}
        ...
        \node[minimum width=\cardwidth, minimum height=\cardheight, draw, rectangle, fill=white, scale=1] at  ({\j * (\cardwidth + 0.12in)}, {-\i * (\cardheight + 0.02in)}) {%
            \begin{itemcard}{\cardtitle}
                \begin{center}
                    ...
                    \begin{tcolorbox}[width=\linewidth, boxrule=1.5pt, colframe=black,  boxsep=0pt, top=0pt, bottom=0pt, left=0pt, right=0pt, enhanced, borderline={0.5mm}{0mm}{black}, interior style={clip, fill overzoom image=\cardbg}]
                        \begin{center}
                ...
                \begin{lowerpart}
                    \begin{adjustbox}{minipage=\linewidth, max height=0.72\cardheight}
                        \carddescfront
                    \end{adjustbox}
                \end{lowerpart}
           \end{itemcard}
